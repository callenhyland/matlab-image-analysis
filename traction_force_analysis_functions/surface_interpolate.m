...
%
%z is a column vector of values associated with those points. It must be
...
for i = 1:X
    for j = 1:Y
        rand_samp = [x,y,z, zeros(length(x),1)];
        %current grid position
        pos = [xi(i,j) yi(i,j)];
        
        %if the grid position is outside of the limits of the randomly
        %sampled data set, set to NaN
        if pos(1)<min_x || pos(2)<min_y || pos(1)>max_x || pos(2)>max_y
            ...
            rand_samp(:,4)= sqrt((rand_samp(:,1)-pos(1)).^2 + (rand_samp(:,2)-pos(2)).^2);
            %sort by ascending distance
            rand_samp = sortrows(rand_samp,4);
            %find closest 'num' points
            
            %use surface fitting to find interpolated displacement at 'pos'
            Xf = rand_samp(1:num,1); Yf = rand_samp(1:num,2); Uf = rand_samp(1:num,3);
            P = zeros(num,6);
            P(:,1) = 1;
            P(:,2) = Xf;
            P(:,3) = Yf;
